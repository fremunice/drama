...
export function InfiniteShortMaxSection({ title }: InfiniteShortMaxSectionProps) {
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-3 md:gap-4">
        {allDramas.filter(Boolean).map((drama: any, index: number) => {
          const id = drama.shortPlayId || drama.id;
          const title = drama.title || "Untitled";
          const cover = drama.cover || "";
          const episodes = drama.totalEpisodes || drama.episodes || 0;
