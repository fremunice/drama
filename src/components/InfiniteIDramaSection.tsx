...
import { useInfiniteIdramaForYou } from "@/hooks/useiDrama";
import { Loader2 } from "lucide-react";

interface InfiniteIDramaSectionProps {
  title: string;
}

export function InfiniteIDramaSection({ title }: InfiniteIDramaSectionProps) {
  const {
    data,
    fetchNextPage,
    hasNextPage,
        ...
        {dramas.map((drama, index) => {
          ...
          return (
            <UnifiedMediaCard
              ...
              link={`/detail/idrama/${id}`}
              episodes={drama.totalEpisodes || drama.episodes || 0}
              topLeftBadge={{ text: "iDrama", color: "#de0202" }}
            />
          );
        })}
      </div>
