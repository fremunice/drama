...
export interface BadgeConfig {
  text: string;
  color?: string;       // Background color (e.g., "#de0202" or "hsl(var(--primary))")
...
export function UnifiedMediaCard({
  ...
  const BADGE_BASE = "px-1 py-0.5 md:px-1.5 rounded font-bold text-white shadow-sm leading-none tracking-wide flex items-center justify-center font-sans text-[10px] md:text-[12px] uppercase";
    ...
    >
      ...
      <div className="aspect-[2/3] relative overflow-hidden rounded-xl bg-muted/20">
        {cover ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-muted/40 text-muted-foreground text-sm">No Image</div>
        )}
          ...
          <div className="flex-1 min-w-0 pr-1 flex justify-start">
            {topLeftBadge && (
              <div
                ...
                style={{
                  ...BADGE_FONT,
                  backgroundColor: topLeftBadge.color || "#de0202",
        ...
        {/* Episode Count */}
        {episodes > 0 && (
          <div className="absolute bottom-1.5 left-1.5 md:bottom-2 md:left-2 flex items-center gap-1 text-[10px] md:text-sm text-white font-medium pointer-events-none">
            <Play className="w-3 h-3 md:w-3.5 md:h-3.5 fill-white" />
            <span>{episodes} Ep</span>
          </div>
        )}

        {/* Bottom Right Badge - Provider Label */}
        {bottomRightBadge && (
          <div className="absolute bottom-1.5 right-1.5 md:bottom-2 md:right-2 pointer-events-none z-10">
            <div
              className={`${BADGE_BASE} backdrop-blur-sm`}
              style={{
                ...BADGE_FONT,
                backgroundColor: bottomRightBadge.isTransparent ? "rgba(0,0,0,0.6)" : (bottomRightBadge.color || "rgba(0,0,0,0.6)"),
                color: bottomRightBadge.textColor || "#FFFFFF"
              }}
            >
              {bottomRightBadge.text}
            </div>
          </div>
        )}
      ...
      <div className="pt-2 md:pt-3 pb-1">
        <h3 className="font-display font-semibold text-sm md:text-base leading-snug line-clamp-2 text-foreground group-hover:text-primary transition-colors">
          {title}
        </h3>
      </div>
    </Link>
