...
export function NetShortHome() {
  ...
  // Filter out excluded sections (only if theatersData has grouped data)
  const filteredGroups = Array.isArray(theatersData?.data)
    ? (theatersData.data as any[]).filter((group: any) => {
      const sectionName = stripEmoji(group.groupName || group.group_name || "").toLowerCase();
      return !EXCLUDED_SECTIONS.some((excluded) => sectionName.includes(excluded));
    })
    : [];

  // Flat items from API (when grouped data not available)
  const flatItems: any[] = theatersData?.items || theatersData?.data || [];

  // Show flat items when no grouped data, or if no groups after filtering
  const showFlatItems = filteredGroups.length === 0 && flatItems.length > 0;

  return (
    <div className="space-y-10">
      {/* For You Section */}
      {forYouData && ((forYouData.data?.length ?? 0) > 0 || (forYouData.items?.length ?? 0) > 0) && (
        <section>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-3 md:gap-4">
            {(forYouData.data || forYouData.items || []).map((drama: any, index: number) => (
              <UnifiedMediaCard
                key={drama.shortPlayId || drama.id || index}
                index={index}
      ...
      {filteredGroups.map((group) => (
        <section key={group.groupId}>
          ...
          {/* Drama Grid */}
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-3 md:gap-4">
            {group.dramas.slice(0, 12).map((drama: any, index: number) => (
              <UnifiedMediaCard
                key={drama.shortPlayId || drama.id || index}
                index={index}
                title={drama.title || "Untitled"}
      ...
      {/* Fallback: Show flat items when no grouped data */}
      {showFlatItems && (
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold font-display text-foreground">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-3 md:gap-4">
            {flatItems.slice(0, 16).map((item: any, index: number) => (
              <UnifiedMediaCard
                key={item.id || item.shortPlayId || index}
                index={index}
                title={item.title || "Untitled"}
                cover={item.cover || ""}
                link={`/detail/netshort/${item.id || item.shortPlayId}`}
