...
export function PlatformSelector() {
  ...
  return (
    <div className="w-full py-4 px-4">
      ...
      <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 max-w-2xl">
        {platforms.map((platform) => (
          <PlatformButton
            key={platform.id}
            platform={platform}
...
function PlatformButton({ platform, isActive, onClick }: PlatformButtonProps) {
  return (
    <button
      onClick={onClick}
      className={`
        relative flex items-center gap-3 px-4 py-2.5 rounded-xl w-full
