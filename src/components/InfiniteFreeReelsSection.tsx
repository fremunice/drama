...
export function InfiniteFreeReelsSection({ title }: InfiniteFreeReelsSectionProps) {
  ...
  // Flatten pages - API returns { code, items: [...], hasMore }
  const allDramas = data?.pages.flatMap((page: any) =>
    (page.items || page.data?.items || []).map((item: any) => ({
      key: item.id || item.key || item.dramaId,
      title: item.title,
      cover: item.cover || item.posterImg || "",
      episode_count: item.episodes || item.episode_count || 0,
      follow_count: item.viewCount || item.follow_count || 0,
    }))
  ) || [];
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-3 md:gap-4">
        {allDramas.map((item, index) => (
          <UnifiedMediaCard
            ...
            topLeftBadge={{ text: "FreeReels", color: "#de0202" }}
