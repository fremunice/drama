...
interface DramaWaveSectionProps {
  title: string;
  dramas?: any[];
  isLoading?: boolean;
  error?: boolean;
...
export function DramaWaveSection({ title, dramas, isLoading, error, onRetry }: DramaWaveSectionProps) {
  if (error) {
    return (
      <section>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-3 md:gap-4">
        {dramas?.map((drama, index) => (
          <UnifiedMediaCard
            ...
            link={`/detail/dramawave/${drama.id || drama.bookId || drama.dramaId}`}
