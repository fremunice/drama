...
export function InfiniteMeloloSection({ title }: InfiniteMeloloSectionProps) {
  ...
  const allBooks = data?.pages.flatMap((page) => page?.items || page?.books || []) || [];
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-3 md:gap-4">
        {allBooks.filter(Boolean).map((book, index) => {
          const bookId = book.book_id || book.id || `melolo-${index}`;
          return (
            <UnifiedMediaCard
              key={bookId}
              index={index}
              title={book.book_name || book.title || "Untitled"}
              cover={book.thumb_url || book.cover || ""}
              link={`/detail/melolo/${bookId}`}
              episodes={book.serial_count || book.episode_count || 0}
