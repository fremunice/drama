...
export default function ShortMaxDetailPage() {
  ...
  // API returns standardized format: { id, title, cover, description, episodes, ... }
  const drama = data as any;
  const episodeCount = drama?.episodes?.length || 0;
  const dramaId = drama?.id || shortPlayId;

  if (error || !drama?.title) {
    return (
            ...
            <div className="relative group">
              ...
              {episodeCount > 0 && (
                <div className="absolute inset-0 rounded-2xl bg-gradient-to-t from-background/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center pb-6">
                  <Link
                    href={`/watch/shortmax/${dramaId}?ep=1`}
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span>{episodeCount} Episode</span>
                  </div>
                </div>

                {/* Tags / Labels */}
                {(drama.tags?.length > 0 || drama.labels?.length > 0) && (
                  <div className="flex flex-wrap gap-2 mt-3">
                    {(drama.tags || drama.labels).map((label: string, idx: number) => (
