...
export async function GET() {
  try {
    const response = await fetch(`${UPSTREAM_API}/${PROVIDER}/hotrank`, {
      headers: HEADERS,
    });

    if (!response.ok) {
      console.warn(`[${PROVIDER} Hotrank] Upstream error:`, response.status);
    ...
    console.error(`${PROVIDER} hotrank error:`, error);
    return NextResponse.json({ code: 200, items: [], hasMore: false });
  }
}