...
export async function GET() {
  try {
    const response = await fetch(`${UPSTREAM_API}/${PROVIDER}/latest`, {
      headers: HEADERS,
    });

    if (!response.ok) {
      console.warn(`[${PROVIDER} Latest] Upstream error:`, response.status);
    ...
    console.error(`${PROVIDER} latest error:`, error);
    return NextResponse.json({ code: 200, items: [], hasMore: false });
  }
}