...
import { useStarShortDetail } from "@/hooks/useStarShort";
...
export default function StarShortWatchPage() {
  ...
  const { data, isLoading } = useStarShortDetail(dramaId || "");

  const handleEpisodeChange = (ep: number, preserveFullscreen = false) => {
    setCurrentEpisode(ep);
    setShowEpisodeList(false);
    if (preserveFullscreen) {
      window.history.replaceState(null, '', `/watch/starshort/${dramaId}?ep=${ep}`);
    } else {
      router.push(`/watch/starshort/${dramaId}?ep=${ep}`);
    }
  };
  ...
  const videoUrl = useMemo(() => {
    ...
    return `/api/proxy/video?source=starshort&url=${encodeURIComponent(rawUrl)}`;
  }, [currentEpisodeData]);
    ...
    <main className="fixed inset-0 bg-black flex flex-col">
      <div className="absolute top-0 left-0 right-0 z-40 h-16 pointer-events-none">
        ...
        <div className="relative z-10 flex items-center justify-between h-full px-4 max-w-7xl mx-auto pointer-events-auto">
          <Link href={`/detail/starshort/${dramaId}`} className="flex items-center gap-2 text-white/90 hover:text-white transition-colors p-2 -ml-2 rounded-full hover:bg-white/10">
