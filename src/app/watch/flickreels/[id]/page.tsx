...
export default function FlickReelsWatchPage() {
  ...
  const { data: detailData, isLoading: detailLoading } = useFlickReelsDetail(dramaId || "");
  ...
  const handleEpisodeChange = (index: number, preserveFullscreen = false) => {
    ...
    if (preserveFullscreen) {
      window.history.replaceState(null, '', `/watch/flickreels/${dramaId}?ep=${index}`);
    } else {
      router.push(`/watch/flickreels/${dramaId}?ep=${index}`);
    }
  };
  ...
  const videoUrl = useMemo(() => {
    // Use hlsUrl from episode API (pre-signed URL), or fallback to videoUrl
    const hlsUrl = currentEpisodeData?.hlsUrl || currentEpisodeData?.hls_url;
    if (hlsUrl) {
      // hlsUrl is a relative path like /api/flickreels/hls?id=...&ep=...&ts=...&sig=...
      return hlsUrl.startsWith("http") ? hlsUrl : hlsUrl;
    }
    ...
    return `/api/proxy/video?source=flickreels&url=${encodeURIComponent(rawUrl)}`;
  }, [currentEpisodeData]);
  ...
  const handleVideoEnded = () => {
    ...
    if (next <= episodes.length - 1) handleEpisodeChange(next, true);
  };
    ...
    <main className="fixed inset-0 bg-black flex flex-col">
      <div className="absolute top-0 left-0 right-0 z-40 h-16 pointer-events-none">
        ...
        <div className="relative z-10 flex items-center justify-between h-full px-4 max-w-7xl mx-auto pointer-events-auto">
          <Link href={`/detail/flickreels/${dramaId}`} className="flex items-center gap-2 text-white/90 hover:text-white transition-colors p-2 -ml-2 rounded-full hover:bg-white/10">
          ...
          <div className="text-center flex-1 px-4 min-w-0">
            ...
            <p className="text-white/80 text-xs drop-shadow-md">{currentEpisodeData?.episodeTitle || `Episode ${currentEpisode + 1}`}</p>
          </div>
      ...
      <div className="flex-1 w-full h-full relative bg-black flex flex-col items-center justify-center">
        <div className="relative w-full h-full flex items-center justify-center">
          {currentEpisodeData && !currentEpisodeData.locked ? (
            ...
            <div className="flex flex-col items-center justify-center text-white/60 gap-4">
              ...
              <p className="text-lg">{currentEpisodeData?.locked ? "Episode terkunci" : "Video tidak tersedia"}</p>
            </div>
          )}
        </div>
        <div className="absolute bottom-20 md:bottom-12 left-0 right-0 z-40 pointer-events-none flex justify-center pb-safe-area-bottom">
          <div className="flex items-center gap-2 md:gap-6 pointer-events-auto bg-black/60 backdrop-blur-md px-3 py-1.5 md:px-6 md:py-3 rounded-full border border-white/10 shadow-lg transition-all scale-90 md:scale-100 origin-bottom">
            <button onClick={() => currentEpisode > 0 && handleEpisodeChange(currentEpisode - 1)} disabled={currentEpisode <= 0} className="p-1.5 md:p-2 rounded-full text-white disabled:opacity-30 hover:bg-white/10 transition-colors">
              <ChevronLeft className="w-4 h-4 md:w-6 md:h-6" />
            </button>
            <span className="text-white font-medium text-xs md:text-sm tabular-nums min-w-[60px] md:min-w-[80px] text-center">Ep {currentEpisode + 1} / {totalEpisodes}</span>
            <button onClick={() => currentEpisode < totalEpisodes - 1 && handleEpisodeChange(currentEpisode + 1)} disabled={currentEpisode >= totalEpisodes - 1} className="p-1.5 md:p-2 rounded-full text-white disabled:opacity-30 hover:bg-white/10 transition-colors">
          ...
          <div className="fixed inset-y-0 right-0 w-72 bg-zinc-900 z-[70] overflow-y-auto border-l border-white/10 shadow-2xl animate-in slide-in-from-right">
            ...
            <div className="p-3 grid grid-cols-5 gap-2">
              {episodes.map((episode: any, idx: number) => (
                <button key={episode.episodeNumber || idx} onClick={() => handleEpisodeChange(idx)}
                  className={`aspect-square flex items-center justify-center rounded-lg text-sm font-medium transition-all ${idx === currentEpisode ? "bg-primary text-white shadow-lg shadow-primary/20" : episode.locked ? "bg-white/5 text-white/30 cursor-not-allowed" : "bg-white/5 text-white/70 hover:bg-white/10 hover:text-white"}`}
                  disabled={episode.locked}>
                  {episode.locked ? <span className="text-xs">🔒</span> : idx + 1}
                </button>
              ))}
            </div>
          </div>
