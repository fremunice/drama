...
import { useMeloloDetail, useMeloloEpisode } from "@/hooks/useMelolo";
...
export default function MeloloWatchPage() {
  const params = useParams<{ bookId: string; episodeNumber: string }>();
  ...
  const { data: detailData, isLoading: detailLoading } = useMeloloDetail(bookId || "");
  const { data: episodeData, isLoading: episodeLoading } = useMeloloEpisode(bookId || "", currentEpisode);

  const handleEpisodeChange = (ep: number, preserveFullscreen = false) => {
    setCurrentEpisode(ep);
    setShowEpisodeList(false);
    if (preserveFullscreen) {
      window.history.replaceState(null, '', `/watch/melolo/${bookId}/${ep}`);
    } else {
      router.push(`/watch/melolo/${bookId}/${ep}`);
    }
  };

  // Get episodes list from detail data
  const episodes = useMemo(() => detailData?.episodes || [], [detailData]);

  // Get video URL from episode API response (detail API has empty videoUrl)
  const videoUrl = useMemo(() => {
    if (!episodeData?.videoUrl) return "";
    const rawUrl = episodeData.videoUrl;
    if (!rawUrl) return "";
    return `/api/proxy/video?source=melolo&url=${encodeURIComponent(rawUrl)}`;
  }, [episodeData]);
    ...
    <main className="fixed inset-0 bg-black flex flex-col">
      <div className="absolute top-0 left-0 right-0 z-40 h-16 pointer-events-none">
        ...
        <div className="relative z-10 flex items-center justify-between h-full px-4 max-w-7xl mx-auto pointer-events-auto">
          <Link href={`/detail/melolo/${bookId}`} className="flex items-center gap-2 text-white/90 hover:text-white transition-colors p-2 -ml-2 rounded-full hover:bg-white/10">
      ...
      <div className="flex-1 w-full h-full relative bg-black flex flex-col items-center justify-center">
        <div className="relative w-full h-full flex items-center justify-center">
          {videoUrl ? (
            <video ref={videoRef} controls autoPlay playsInline crossOrigin="anonymous" onEnded={handleVideoEnded} className="w-full h-full object-contain max-h-[100dvh]" />
          ) : (
          ...
          <div className="fixed inset-y-0 right-0 w-72 bg-zinc-900 z-[70] overflow-y-auto border-l border-white/10 shadow-2xl animate-in slide-in-from-right">
            ...
            <div className="p-3 grid grid-cols-5 gap-2">
              {episodes.map((ep: any, idx: number) => (
                <button key={idx} onClick={() => handleEpisodeChange(ep.episodeNumber || idx + 1)}
                  className={`aspect-square flex items-center justify-center rounded-lg text-sm font-medium transition-all ${ep.episodeNumber === currentEpisode || idx + 1 === currentEpisode ? "bg-primary text-white shadow-lg shadow-primary/20" : "bg-white/5 text-white/70 hover:bg-white/10 hover:text-white"}`}>
