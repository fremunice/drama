...
export default function GoodShortWatchPage() {
  ...
  const { data, isLoading } = useGoodShortDetail(bookId || "");

  const handleEpisodeChange = (ep: number, preserveFullscreen = false) => {
    setCurrentEpisode(ep);
    setShowEpisodeList(false);
    if (preserveFullscreen) {
      window.history.replaceState(null, '', `/watch/goodshort/${bookId}/${ep}`);
    } else {
      router.push(`/watch/goodshort/${bookId}/${ep}`);
    }
  };

  const episodes = useMemo(() => data?.episodes || [], [data]);
  const currentEpisodeData = useMemo(() => episodes[currentEpisode - 1] || episodes[0], [episodes, currentEpisode]);
  ...
  const videoUrl = useMemo(() => {
    ...
    const rawUrl = currentEpisodeData.videoUrl.startsWith("http") 
      ...
      : `https://api.anichin.bio${currentEpisodeData.videoUrl}`;
    return `/api/proxy/video?source=goodshort&url=${encodeURIComponent(rawUrl)}`;
  }, [currentEpisodeData]);
  ...
  const dramaTitle = data?.book?.title || data?.book?.bookName || "Drama";
    ...
    <main className="fixed inset-0 bg-black flex flex-col">
      <div className="absolute top-0 left-0 right-0 z-40 h-16 pointer-events-none">
        ...
        <div className="relative z-10 flex items-center justify-between h-full px-4 max-w-7xl mx-auto pointer-events-auto">
          <Link href={`/detail/goodshort/${bookId}`} className="flex items-center gap-2 text-white/90 hover:text-white transition-colors p-2 -ml-2 rounded-full hover:bg-white/10">
