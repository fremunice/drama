...
export default function FreeReelsWatchPage() {
  ...
  // Normalize URL: if activeEpisodeId doesn't match any episode, redirect to first episode's actual ID
  useEffect(() => {
    if (episodes.length > 0 && currentIndex === -1 && activeEpisodeId !== episodes[0]?.id) {
      router.replace(`/watch/freereels/${bookId}/${episodes[0].id}`);
    }
  }, [episodes, currentIndex, activeEpisodeId, bookId, router]);
        ...
        <div className="relative w-full h-full flex items-center justify-center">
          {currentVideoUrl ? (
            <video
              ...
              ref={videoRef}
              src={useProxy ? `/api/proxy/video?source=freereels&url=${encodeURIComponent(currentVideoUrl)}` : currentVideoUrl}
              controls
              autoPlay
