...
// Response type for ReelShort homepage API
interface ReelShortForYouResponse {
  items?: Drama[];
  rows?: Drama[];
  hasMore?: boolean;
  total?: number;
...
export function useInfiniteReelShortDramas() {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 1 }) =>
      fetchJson<ReelShortForYouResponse>(`${REC_API_BASE}/homepage?page=${pageParam}`),
    initialPageParam: 1,
...
import { useDramaBiteDetail } from "./useDramaBite";
import { useDramaWaveDetail } from "./useDramaWave";
import { useFlickReelsDetail } from "./useFlickReels";
import { useIdramaDetail } from "./useiDrama";
import { useStarShortDetail } from "./useStarShort";

export { useDramaBiteDetail, useDramaWaveDetail, useFlickReelsDetail, useIdramaDetail, useStarShortDetail };
...
export function useSearchDramas(query: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return fetchJson<SearchResult[]>(`${API_BASE}/search?q=${encodeURIComponent(normalizedQuery)}`);
    },
...
export function useDubindoDramas() {
  return useQuery({
    ...
    queryFn: () => fetchJson<Drama[]>(`${API_BASE}/hotrank`),
    staleTime: 1000 * 60 * 5,
  });
}
